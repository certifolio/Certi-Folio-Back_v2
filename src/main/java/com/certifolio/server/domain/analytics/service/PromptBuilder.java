...
public class PromptBuilder {
    ...
    private void buildRoleSection(StringBuilder sb) {
        sb.append("[역할]\n")
                .append("당신은 한국 IT/SW 채용 시장에서 10년 이상 경력을 가진 취업 포트폴리오 전문 컨설턴트입니다.\n")
                .append("평가 대상은 컴퓨터공학/소프트웨어 관련 전공 대학생 또는 신입 구직자입니다.\n\n");
    }

    private void buildPreferenceSection(StringBuilder sb, CareerPreference preference) {
        sb.append("[희망 진로]\n");
        if (preference != null) {
    ...
    private void buildPortfolioSection(
            StringBuilder sb,
            List<EducationResponseDTO> educations,
            List<CareerResponseDTO> careers,
            List<CertificateResponseDTO> certificates,
    ...
    private void buildCareers(StringBuilder sb, List<CareerResponseDTO> careers) {
    ...
    private void buildProjects(StringBuilder sb, List<ProjectResponseDTO> projects) {
        sb.append(String.format("\n## 프로젝트 (%d건)\n", projects.size()));
        if (projects.isEmpty()) {
    ...
    private void buildActivities(StringBuilder sb, List<ActivityResponseDTO> activities) {
        sb.append(String.format("\n## 대외활동 (%d건)\n", activities.size()));
    ...
    private void buildRubricSection(StringBuilder sb) {
        sb.append("\n[카테고리별 평가 루브릭]\n\n");
        ...
        sb.append("[가중 평균 공식]\n");
        sb.append("overallScore = 실무경력×0.25 + 프로젝트경험×0.30 + 자격증어학×0.15 + 학점전공×0.15 + 대외활동×0.15\n");
        sb.append("(소수점 반올림하여 정수로)\n\n");
    }

    private void buildResponseGuideSection(StringBuilder sb) {
        sb.append("[응답 지침]\n");
